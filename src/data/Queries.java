...
public class Queries {
	...
	public static final String GET_GAME_VS_OPPONENT =
			...
			+ "join tornei t on p.codtorneo = t.codtorneo "
			+ "where (g_bianco.idgiocatore = ? and "
			+ "(g_nero.nome like concat ('%', ?, '%') or g_nero.cognome like concat('%', ?, '%'))) "
			+ "or "
			+ "(g_nero.idgiocatore = ? and "
			+ "(g_bianco.nome like concat('%', ?, '%') or g_bianco.cognome like concat('%', ?, '%'))) "
			+ "and p.data < ? "
			+ "order by p.data "
			+ "limit 20 ";
